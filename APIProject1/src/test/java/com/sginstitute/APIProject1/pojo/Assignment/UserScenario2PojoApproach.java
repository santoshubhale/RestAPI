...
public class UserScenario2PojoApproach {
...
public void UpdateUserPut()
{
	try
	{
	System.out.println("Update First name of user using PUT method");
	RestAssured.baseURI="http://localhost:8083/springboot-sgsoftwaretestinginstitute-sgtesting/users/"+user_id;
	RequestSpecification httpRequest=RestAssured.given();
	//Create POJO class
	Users userobj=new Users("banasawadi nagar", null, null, null, null, null, null, 0);
	ObjectMapper mapper=new ObjectMapper();
	String jsonuser=mapper.writerWithDefaultPrettyPrinter().writeValueAsString(userobj);
	httpRequest.header("content-type", "application/json");
	httpRequest.body(jsonuser);
